...
# ──────────────────────── HELPER: LOAD 3D MODEL ──────────────────
def create_rocket_visual(parent_entity, model_filename, color_tint, size=4.0):
    ...
    if vis.model:
        vis.rotation_x = 0 # Putar 90 derajat karena biasanya model roket Y-up, kita butuh Z-forward
...
defender_missile_lifetime = Slider(min=1, max=100, default=cfg.defender_missile_lifetime, step=1, text='Def. Lifetime', parent=control_panel, position=(col1_x, start_y - 7*gap_y), scale=sl_scale)
...
defender_detect_radius = Slider(min=100, max=3000, default=cfg.defender_detect_radius, step=10, text='Detect Radius', parent=control_panel, position=(col2_x, start_y - 6*gap_y), scale=sl_scale)
...
class YellowAttackMissile:
    def __init__(self):
        ...
        self.body = Entity(position=self.origin)
        # self.trail = []
        # for j in range(8):
        #     tr = Entity(model='sphere', scale=0.8,
        #                 color=color.yellow)
        #     self.trail.append({'e': tr, 'pos': Vec3(self.origin)})
        create_rocket_visual(self.body, 'Rocket_v4.obj', color.yellow, size=4.5)
...
class AttackMissile:
    def __init__(self):
        ...
        self.body = Entity()
        create_rocket_visual(self.body, 'Rocket_v2.obj', color.red, size=4.0)
...
class InterceptMissile:
    def __init__(self, target_atk, start_pos, team_color, team_name):
        ...
        self.body = Entity()
        create_rocket_visual(self.body, 'Rocket_v4.obj', team_color, size=3.0)
    ...
    def advance(self, dt):
        ...
        # 2. Estimasi Kecepatan Target (Vektor)
        # Kita ambil perubahan posisi target antara frame ini dan frame sangat singkat kedepan
        t_sample = 0.01
        ...
        # 3. MENCARI TITIK TEMU (INTERCEPT POINT)
        # Jarak relatif
        rel_pos = target_pos - my_pos
        rel_dist = rel_pos.length()

        # Kecepatan penutupan (Closing Speed)
        # Rumus sederhana: Jarak / (Kecepatan kita + Kecepatan mendekat target)
        # Ini memastikan kita tidak membidik target di titik jatuh, tapi di jalur lintasannya
        time_to_intercept = rel_dist / (self.speed + target_velocity_vec.length())
        ...
        self.current_dir = lerp(self.current_dir, desired_dir, dt * 5.0).normalized()
